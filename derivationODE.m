...
syms q1f(t);            % generalized Coordinate describes the cart position 
syms q2f(t);            % generalized Coordinate which describes the pole angle
syms g l m1 m2 d1;      % constants: gravitation, lenth of pole, mass of cart, mass of pendulum, viscous friction of the cart 

% Mass matrices Moment 
...
% Rod's angle position 
y2=[  q1f-l*sin(q2f); ...
       l*cos(q2f);  ...
          q2f          ];

% Positions Derivation
y1d=diff(y1,t);
y2d=diff(y2,t);


% Energy Equations
T=0.5*(y1d.'*M1*y1d + y2d.'*M2*y2d);      %Kinetic Energy
G_Vector=[0; g; 0].';                     % gravity is only on y-axis
U=G_Vector*M1*y1  +  G_Vector*M2*y2       % Potential Energy
...
% Euler-Lagrange Equation With viscous Damping
...
% Solve for the highest Derivatives
eq=lhs== 0;
...
% State Space Representation: Statevector = [x, x-dot, theta, theta-dot]
statevector=[q1; q1d; q2; q2d];
f=[q1d; q1dd; q2d; q2dd];         


% Computation of the Jacobians Matrices A & B
A=jacobian(f,statevector);                   
B=jacobian(f,u);


% Evaluation Of the Jacobians at fixed point = [0 0 0 0]
